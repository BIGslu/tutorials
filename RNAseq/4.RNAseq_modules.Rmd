...
We extract gene significant for the interaction term as well as those significant for virus or asthma alone.
...
This results in `r length(genes.all)` for using in module building. Note that this example data set contains only 1000 genes out of the roughly 14,000 protein coding genes usually seen in an RNAseq data set. Thus, the genes used in module analysis here are many fewer than would be seen in a full data set.
...
As the name suggest, minimum module size is the minimum number of correlated genes needed to make a module. If you have a group of correlated genes that do not make the minimum, those genes get redistributed, thus changing the overall network.
...
Once you've finalized your modules, you can assess their expression similar to individual genes. Here, we use mean expression of genes in modules. You may use the module eigen values as well (contained in `mod.p14$MEs`). We generally see similar results with the two methods and chose mean module expression because the values are more directly interpretable in relation to actual expression.
