...
Example data were obtained from virus-stimulated human plasmacytoid dendritic cells. Actual patient identifiers and metadata have been altered for this tutorial. You do not need to download any data; they are provide as example data in the `kimma` package.
...
::: {.callout-note} 
the total "co-variates" listed in the above message includes non-reference levels of the main effects (HRV) plus variables listed as `covar` (sex).
:::
